...
<body class="bg-slate-100 antialiased" style="font-family: 'Inter', sans-serif;">
    ...
    <aside id="sidebar" class="sidebar overflow-y-auto">
        ...
        <!-- User Card at Bottom -->
        <div class="mt-auto px-2 py-3 border-t border-slate-800 user-card">
            <div class="user-card-inner flex items-center justify-between gap-2 px-1">
                <!-- Avatar & User Info: Klik Langsung ke Profil Saya -->
                <a href="{{ route('profile.edit') }}" 
                   title="Profil Saya ({{ Auth::user()->name }})"
                   class="flex items-center gap-3 min-w-0 flex-1 p-1 rounded-xl hover:bg-slate-800 transition group text-left no-underline shrink-0">
                    <div class="w-9 h-9 rounded-full bg-indigo-600 group-hover:bg-indigo-500 flex items-center justify-center text-white font-bold text-xs shrink-0 shadow transition">
                        {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                    </div>
                    <div class="user-info min-w-0 flex-1">
                        <div class="text-white font-semibold text-sm truncate group-hover:text-indigo-300 transition">{{ Auth::user()->name }}</div>
                        <div class="text-slate-400 text-xs truncate">{{ Auth::user()->email }}</div>
                    </div>
                </a>

                <!-- Tombol Keluar: Selalu Tampil (Bahkan Saat Sidebar Collapsed) -->
                <form method="POST" action="{{ route('logout') }}" class="shrink-0">
                    @csrf
                    <button type="submit" 
                            title="Keluar dari Aplikasi"
                            class="text-slate-400 hover:text-rose-400 hover:bg-rose-500/20 transition p-2 rounded-xl cursor-pointer flex items-center justify-center">
    ...
    <div id="main-content" class="main-content">
        ...
        <div class="topbar">
            ...
            <div class="flex items-center gap-3 ml-auto">
                ...
                <!-- User Avatar & Profile Dropdown (Clickable Header Pill) -->
                <div class="relative" x-data="{ open: false }">
                    <button @click.stop="open = !open" 
                            type="button"
                            class="flex items-center gap-2.5 p-1 pl-2.5 pr-2 rounded-full bg-slate-50 hover:bg-slate-100 border border-slate-200 transition text-left cursor-pointer group shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20">
                        <span class="text-xs font-semibold text-slate-700 max-w-[130px] truncate hidden sm:inline-block">
                            {{ Auth::user()->name }}
                        </span>
                        <div class="w-8 h-8 rounded-full bg-indigo-600 group-hover:bg-indigo-700 flex items-center justify-center text-white font-bold text-xs shadow-sm shrink-0 transition">
                            {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                        </div>
                        <svg class="w-3.5 h-3.5 text-slate-400 group-hover:text-slate-600 transition shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </button>
                    ...
                    <div x-show="open" 
                         ...
                         style="display: none;"
                         class="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-200 py-2 z-50 overflow-hidden">
                        <div class="px-4 py-2.5 bg-slate-50/80 border-b border-slate-100">
                            ...
                            <span class="inline-block mt-1 px-2 py-0.5 text-[10px] font-bold uppercase rounded-full bg-indigo-100 text-indigo-700">
                                {{ Auth::user()->getRoleNames()->first() ?? 'User' }}
                            </span>
                        </div>
                        <a href="{{ route('profile.edit') }}" class="flex items-center gap-2.5 px-4 py-2.5 text-xs text-slate-700 hover:bg-indigo-50 hover:text-indigo-600 font-semibold transition">
                            <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                            Profil Saya
                        </a>
                        <a href="{{ route('guide') }}" class="flex items-center gap-2.5 px-4 py-2.5 text-xs text-slate-700 hover:bg-indigo-50 hover:text-indigo-600 font-semibold transition">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="w-full flex items-center gap-2.5 px-4 py-2.5 text-xs text-rose-600 hover:bg-rose-50 font-bold transition text-left cursor-pointer">

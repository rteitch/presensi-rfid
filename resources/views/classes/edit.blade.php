<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Edit Data Kelas</h1>
                <p class="text-sm text-slate-500 mt-0.5">Ubah nama kelas, wali kelas, atau tahun ajaran.</p>
            </div>
            ...
            <form method="POST" action="{{ route('classes.update', $class) }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="nama_kelas" value="{{ old('nama_kelas', $class->nama_kelas) }}" required class="form-input">
                    ...
                    <div>
                        ...
                        <select name="academic_year_id" required class="form-input">
                            ...
                            @foreach($academicYears as $y)
                                <option value="{{ $y->id }}" {{ old('academic_year_id', $class->academic_year_id) == $y->id ? 'selected' : '' }}>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="wali_kelas_id" class="form-input">
                            ...
                            @foreach($gurus as $g)
                                <option value="{{ $g->id }}" {{ old('wali_kelas_id', $class->wali_kelas_id) == $g->id ? 'selected' : '' }}>{{ $g->name }} ({{ $g->email }})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
